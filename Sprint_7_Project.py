...
# Random guessing gives accuracy just under 60%. Lower than all other ML models tested. However, the Random Forest Model did not score so well on its final test. It only score a 60%, and since about 70% of all the plans are not ultra, a model that just assumed Not Ultra would score higher than the random forest model. Likewise, the Logistics Regression only scored 69% on its test which is the same accuracy as assuming all plans are not ultra. Neither of those models were very effective. The Decision Tree Classifier on the other hand scored over 80% accuracy. Although it takes a while to run, it produces strong results.
